    ...
    <div className="card glass" style={{ marginBottom: '24px' }}>
      <form onSubmit={handleChannelSearch}>
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button type="submit" className="action-btn" disabled={isSearchingChannels} style={{ minWidth: '100px' }}>
        ...
        <div style={{ marginTop: '20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px' }}>
          {searchResults.map((user) => (
            <button 
              ...
              type="button"
            >
              <img src={user.profileImageURL} alt={user.displayName} style={{ width: '40px', height: '40px', borderRadius: '50%' }} />
