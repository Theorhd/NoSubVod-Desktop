...
const getStatusDisplay = (status: any) => {
  ...
  if (status && typeof status === 'object' && 'Error' in status)
    return { label: `Erreur: ${status.Error}`, icon: <AlertCircle size={16} />, color: '#ff4a4a' };
...
const DownloadPlayer = React.memo(
  ({
    ...
    const source = useMemo((): NSVMediaSource | null => {
      ...
      if (low.endsWith('.m3u8')) return { src: url, type: 'application/x-mpegurl' };
    ...
    return (
      <div className="download-player-shell card">
        ...
        <NSVPlayer
          ...
          onError={() =>
            setError('Lecture impossible: format non supporté ou fichier inaccessible.')
          }
        />
...
const QueueItem = React.memo(
  ({ dl, knownFile }: { dl: ActiveDownload; knownFile?: DownloadedFile }) => {
    ...
    const thumbnail = dl.progress > 0 ? knownFile?.metadata?.previewThumbnailURL || null : null;
          ...
          <div className="download-queue-main">
            <div className="download-title-row">
              ...
              <span className="queue-status-dot" style={{ color: statusInfo.color }}>
                {statusName === 'Downloading' ? <Pause size={14} /> : statusInfo.icon}
              </span>
            </div>
            <div className="download-queue-subline">{dl.current_time || 'En cours'}</div>
...
export default function Downloads() {
  ...
  const scrollQueue = useCallback((direction: 'left' | 'right') => {
    queueRef.current?.scrollBy({ left: direction === 'left' ? -360 : 360, behavior: 'smooth' });
  }, []);
          ...
          <section className="download-section">
            <div className="download-section-head">
              ...
              <div className="queue-nav-group">
                <button type="button" className="queue-nav-btn" onClick={() => scrollQueue('left')}>
                  <ChevronLeft size={16} />
                </button>
                <button
                  type="button"
            ...
            <div className="download-queue-track" ref={queueRef}>
              {activeDownloads.map((dl) => (
                <QueueItem key={dl.vod_id} dl={dl} knownFile={knownVodById[dl.vod_id]} />
              ))}
            </div>
          </section>
        )}
